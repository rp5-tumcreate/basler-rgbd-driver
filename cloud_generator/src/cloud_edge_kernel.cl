...
__kernel void cloud_edge_kernel(__constant struct Params* cameraParams, __global ushort* depthImage, __global uchar* edgeImage, __global float* cloudOutput)
{
    ...
    if(d != 0 && g != 0){
        ...
        if(xt < -cameraParams->xRange || xt > cameraParams->xRange || yt < -cameraParams->yRange || yt > cameraParams->yRange || zt < cameraParams->zRange || zt > 4.0){
